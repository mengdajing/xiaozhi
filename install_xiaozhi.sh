...
check_installed() {
    # 检查目录是否存在且非空
    if [ -d "/home/xiaozhi-server/" ] && [ "$(ls -A /home/xiaozhi-server/)" ]; then
        DIR_CHECK=1
    else
        DIR_CHECK=0
    fi
...
if check_installed; then
    if whiptail --title "已安装检测" --yesno "检测到小智服务端已安装，是否进行升级？" 10 60; then
        ...
        docker compose -f /home/xiaozhi-server/docker-compose_all.yml down
        ...
        # 备份原有配置文件
        mkdir -p /home/xiaozhi-server/backup/
        if [ -f /home/xiaozhi-server/data/.config.yaml ]; then
            cp /home/xiaozhi-server/data/.config.yaml /home/xiaozhi-server/backup/.config.yaml
            echo "已备份原有配置文件到 /home/xiaozhi-server/backup/.config.yaml"
        fi
        
        # 下载最新版配置文件
        check_and_download "/home/xiaozhi-server/docker-compose_all.yml" "https://ghfast.top/https://raw.githubusercontent.com/xinnan-tech/xiaozhi-esp32-server/refs/heads/main/main/xiaozhi-server/docker-compose_all.yml"
        check_and_download "/home/xiaozhi-server/data/.config.yaml" "https://ghfast.top/https://raw.githubusercontent.com/xinnan-tech/xiaozhi-esp32-server/refs/heads/main/main/xiaozhi-server/config_from_api.yaml"
        
        # 启动Docker服务
        echo "开始启动最新版本服务..."
        # 升级完成后标记，跳过后续下载步骤
        UPGRADE_COMPLETED=1
        docker compose -f /home/xiaozhi-server/docker-compose_all.yml up -d
    else
...
echo "开始创建安装目录..."
# 检查并创建数据目录
if [ ! -d /home/xiaozhi-server/data ]; then
    mkdir -p /home/xiaozhi-server/data
    echo "已创建数据目录: /home/xiaozhi-server/data"
else
    echo "目录xiaozhi-server/data已存在，跳过创建"
fi

# 检查并创建模型目录
if [ ! -d /home/xiaozhi-server/models/SenseVoiceSmall ]; then
    mkdir -p /home/xiaozhi-server/models/SenseVoiceSmall
    echo "已创建模型目录: /home/xiaozhi-server/models/SenseVoiceSmall"
else
...
echo "开始下载语音识别模型"
# 下载模型文件
MODEL_PATH="/home/xiaozhi-server/models/SenseVoiceSmall/model.pt"
...
if [ -n "$SECRET_KEY" ]; then
    python3 -c "
import sys, yaml; 
config_path = '/home/xiaozhi-server/data/.config.yaml'; 
